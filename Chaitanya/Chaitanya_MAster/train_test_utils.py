...
def train_one_step(model, optimizer, x, y_true, input_len, label_len, y_strings):
    ...
    input_len = np.expand_dims(input_len, axis = 1)
    label_len = np.expand_dims(label_len, axis = 1)
   # print("input_len: ",input_len,"label_len : " , label_len, "IN TRAIN ONE STEP")
...
def model_fit(model, optimizer, train_ds, manager, ckpt, val_ds = None,epochs=20):
    ...
    for epoch in range(epochs):
        ...
        for inputs, y in train_ds:
            x, y_strings, ip_len, label_len = inputs
           # print(type(x),type(y),type(ip_len), type(label_len),type(y_strings))
            step += 1
            ...
            first = total//max_batch
            start =0
            stop = start+first
           # print(start,stop)
           # tf.print("BATCH START", total,max_batch)
            for batches in range(max_batch+1):
                    if batches == max_batch:
                        stop = len(x)
                    if stop - start >=2:
                       # print(batches,max_batch,start,stop,total)
                        xx = x[start:stop]
                        yy = y[start:stop]
                        ip_lenn = ip_len[start:stop]
                   # print(type(ip_lenn))
                        label_lenn = label_len[start:stop]
                        y_stringss = y_strings[start:stop]
                   # print(type(xx), type(yy), type(ip_lenn),type(label_lenn),type(y_stringss))

                    

                        loss, accuracy = train_one_step(model, optimizer, xx,yy, ip_lenn, label_lenn, y_stringss)
                        epoch_loss += loss
